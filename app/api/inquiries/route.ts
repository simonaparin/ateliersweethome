...
const allowedTypes = new Set(["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/acad", "image/vnd.dwg", "image/vnd.dxf"]);
...
function isAllowed(file: File) { return file.type.startsWith("image/") || file.type.startsWith("video/") || allowedTypes.has(file.type); }
...
async function telegram(token: string, method: string, body: BodyInit) {
...
export async function POST(request: Request) {
  ...
  if (!message || !contact || !pageTitle || !pageUrl) return NextResponse.json({ ok: false }, { status: 400 });
  ...
  try {
    await telegram(token, "sendMessage", JSON.stringify({ chat_id: chatId, text: telegramMessage, parse_mode: "HTML", disable_web_page_preview: true }));
