...
export function LocalizedServiceLayout({ locale, service, hero, page, images, check, checkTitle, checkText, situations, scope, workshop, start, faq, form, formContent, labels }: any) {
  ...
  return <><Header locale={locale} activePath={`/${service}`} contacts={contactConfig} content={{ brand: "Atelier Sweet Home", writeLabel: locale === "en" ? "Contact us" : "დაგვიკავშირდით" }} /><main className="localized-page">
    {service === "reconstruction" ? <>
      ...
      {situationsBlock}{page.decisions ? <Block eyebrow={labels.decisions} title={page.decisions.title}><ul className="scope-list">{page.decisions.items.map((item: string) => <li key={item}>{item}</li>)}</ul><p className="note">{page.decisions.note}</p></Block> : null}{scopeBlock}<ProfessionalApproach content={professionalApproach[localizedLocale].reconstruction} /><RemoteCollaboration content={remoteCollaboration[localizedLocale].reconstruction} />
