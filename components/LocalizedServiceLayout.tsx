...
function Block({ id, eyebrow, title, text, dark = false, children }: { id?: string; eyebrow: string; title?: string; text?: string; dark?: boolean; children?: React.ReactNode }) {
  return <section id={id} className={`section ${dark ? "split-section dark-check-section" : ""}`}><div className="section-heading"><p className="eyebrow">{eyebrow}</p>{title ? <h2>{title}</h2> : null}{text ? <p>{text}</p> : null}</div>{children}</section>;
}

export function LocalizedServiceLayout({ locale, service, hero, heroImage, page, images, check, checkTitle, checkText, situations, scope, workshop, start, faq, form, formContent, labels }: any) {
  ...
  const relatedBlock = <Block eyebrow={labels.related} title={labels.related} text={labels.relatedText}><div className={`direction-list direction-list--${locale}`}>{related.map(([label, href]) => <a className="direction-row" href={`/${locale}/${href}`} key={href}><span>{label}</span></a>)}</div></Block>;
  const contactBlock = <section className="section contact-section"><div className="contact-copy"><p className="eyebrow">{labels.contact}</p><h2>{form.title}</h2><p>{form.text}</p><ContactLinks contacts={contactConfig} /></div><InquiryForm content={formContent} locale={localizedLocale} /></section>;
  const heroBlock = (className: string) => <section className={className}><div className="hero-copy"><p className="eyebrow">Tbilisi · Kakheti · Georgia</p><h1>{hero.title}</h1><p className="hero-phrase">{hero.mainPhrase}</p><p className="hero-text">{hero.text}</p><div className="hero-actions"><a className="primary-button" href="#contact-form">{hero.primaryCta}</a><a className="secondary-link" href="#work-start">{hero.secondaryCta ?? labels.start}</a></div></div>{service === "reconstruction" ? <ReconstructionHeroCollage houseAlt={hero.title} interiorAlt={hero.title} verandaAlt={hero.title} /> : <figure className="hero-image hero-main-photo"><Image src={(heroImage ?? images[0]).src} alt={(heroImage ?? images[0]).alt ?? hero.title} width={1320} height={980} priority sizes="(max-width: 900px) 100vw, 48vw" /></figure>}</section>;
  ...
  return <><Header locale={locale} activePath={`/${service}`} contacts={contactConfig} content={{ brand: "Atelier Sweet Home", writeLabel: locale === "en" ? "Contact us" : "დაგვიკავშირდით" }} /><main className={`localized-page localized-page--${locale}`}>
    {service === "reconstruction" ? <>
      ...
      <Block eyebrow={labels.details} title={detailsTitle} text={detailsText}><ReconstructionInteriorCollage verandaAlt={hero.title} plantsAlt={hero.title} buffetAlt={hero.title} /></Block>
    ...
    {service === "roof" ? <>
      ...
      {situationsBlock}<Block id="roof-check" eyebrow={labels.inspection} title={checkTitle} text={checkText} dark><ul className="check-list">{check.map((item: string) => <li key={item}>{item}</li>)}</ul></Block>{scopeBlock}
