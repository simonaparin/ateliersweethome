...
export function MessageComposer({ copy, pageTitle }: Props) {
  ...
  return (
    <form className="message-composer" onSubmit={onSubmit}>
      ...
      <textarea name="message" defaultValue={copy.message} aria-label="Message" rows={5} required />
      ...
      {files.length > 0 ? <ul className="message-composer__files">{files.map((file, index) => <li key={`${file.name}-${index}`}><span>{file.name}</span><button type="button" onClick={() => removeFile(index)} aria-label={`Remove ${file.name}`}>×</button></li>)}</ul> : null}
